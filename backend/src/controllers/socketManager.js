import { Server } from "socket.io"
	...
	export const connectToSocket = (server) => {
		...
		io.on("connection", (socket) => {
				...
				if (messages[path] !== undefined) {
					for (let a = 0; a < messages[path].length; ++a) {
						io.to(socket.id).emit("chat-message", messages[path][a]['data'],
							messages[path][a]['sender'], messages[path][a]['socket-id-sender'], messages[path][a]['replyTo'])
					}
				}
			}
			...
			socket.on("chat-message", (data, sender, replyTo) => {
				...
				if (found === true) {
					...
					messages[matchingRoom].push({ 'sender': sender, "data": data, "socket-id-sender": socket.id, "replyTo": replyTo })
					connections[matchingRoom].forEach((elem) => {
						io.to(elem).emit("chat-message", data, sender, socket.id, replyTo)
					})
				}
			})
			...
			socket.on("typing", (username) => {
				...
				if (found === true) {
					connections[matchingRoom].forEach((elem) => {
						if (elem !== socket.id) {
							io.to(elem).emit("typing", username, socket.id);
						}
					})
				}
			})
